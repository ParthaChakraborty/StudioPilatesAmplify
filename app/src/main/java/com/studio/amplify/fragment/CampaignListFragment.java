...
public class CampaignListFragment extends BaseFragment implements LoadingInterface, ParticipateClick {
    ...
    public void onRedirectFromMyCampaignsAndChallenges(ChallengeCampaignListItem challengeCampaignListItem, int userId, String type) {
        ...
        args.putString("From","MyCampaigns");*/
    ...
    public void getCampaignList() {
        ...
        new GetServiceCall(Urls.CAMPAIGN_LIST + loginCredentials.getUserId(), GetServiceCall.TYPE_JSONOBJECT) {
            @Override
            public void response(String response) {
                try {
                    JSONObject jsonObject = new JSONObject(response);
                    Log.d("***camResp", jsonObject.toString());
                    System.out.println(" camObject : " + jsonObject.toString());
                    System.out.println("camUrl : " + Urls.CAMPAIGN_LIST + loginCredentials.getUserId());
                    ...
                    if(jsonArray.length() > 0) {
                        for (int z = 0; z < jsonArray.length(); z++) {
                            ...
                            challengeListItem.setId(jsonObject3.getInt("ID"));
                            challengeListItem.setHeading(jsonObject3.getString("Heading"));
                            challengeListItem.setStartDate(jsonObject3.getString("Sdate"));
                            challengeListItem.setEndDate(jsonObject3.getString("Edate"));
                            challengeListItem.setClassBuy(jsonObject3.getString("noofclassbuy"));
                            challengeListItem.setClassOpted(jsonObject3.getString("classcomplte"));
                            challengeListItemArrayList.add(challengeListItem);
                        }
                    } else {
                        tvNoHeading.setVisibility(View.VISIBLE);
                        tvNoHeading.setText("You have not participated in any campaign");
                    }
                    challengeAndCampaignAdapter = new ChallengeCampaignListAdapter(challengeListItemArrayList, getActivity(),CampaignListFragment.this,loginCredentials.getUserId(),"CA",CampaignListFragment.this);
                    ...
                    mealPlanText.setText(R.string.my_campaigns);
